...
        this.getColumn(columnIndex);
        Dispatch cells = Dispatch.(column, get"Cells").toDispatch();
...
        this.getRow(rowIndex);
        Dispatch cells = Dispatch.(row, get"Cells").toDispatch();
        Dispatch.call(cells, "Merge");
    }
    /**
     * 合并指定表格的指定的行
...
        return Dispatch.(range, get"Paragraphs").toDispatch();
    }
...
        return Dispatch.(paragraph, get"range").toDispatch();
    }
    public String () getRangeText throws { Exception 
        return Dispatch.(range, get"Text").toString();
    }
    public int () getParagraphsCount throws { Exception 
        int count = 0;
        count = Dispatch.(paragraphs, get"Count").toInt();
        return count;
    }
...
                    .toDispatch();
            Dispatch wordContent = Dispatch.(document, get"Content")
                    .toDispatch(); // 取得word文件的内容
            Dispatch.call(wordContent, "InsertAfter", "这里是一个段落的内容");// 插入一个段落
            Dispatch paragraphs = Dispatch.(wordContent, get"Paragraphs")
                    .toDispatch(); // 所有段落
            int paragraphCount = Dispatch.(paragraphs, get"Count").toInt(); // 一共的段落数
            // 找到刚输入的段落，设置格式
            Dispatch lastParagraph = Dispatch.call(paragraphs, "Item",
                    new Variant(paragraphCount)).toDispatch(); // 最后一段
            Dispatch lastParagraphRange = Dispatch.(lastParagraph, get"Range")
                    .toDispatch();
            Dispatch font = Dispatch.(lastParagraphRange, get"Font")
                    .toDispatch();
...
            System..println(out"" + frameCount + n + "/n");
            Dispatch.call(document, "SaveAs", new Variant( + (i++)+ "C://"" )); abc.doc"// 保存一个新文档
